...
public sealed class Steps
{
    ...
    public async Task ThenExpressionTestStep1(IExpression expression)
    {
        Assert.AreEqual(3, await expression.ExecuteAsync<int>());
    }

    [Then(@"expression test step 2 '([^']*)'")]
    public async Task ThenExpressionTestStep2(IExpression expression)
    {
        Assert.AreEqual("1+2", await expression.ExecuteAsync<string>());
    }

    [Then(@"expression test step 3 '([^']*)'")]
    public async Task ThenExpressionTestStep3(IExpression expression)
    {
